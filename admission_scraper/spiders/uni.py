...
class UniSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for link in self.link_extractor.extract_links(response):
            ...
            if (
                word_matches
                and remove_trailing_slash(link.url)
                not in res["potential_admission_links"]
            ):
                res["potential_admission_links"].append(remove_trailing_slash(link.url))
